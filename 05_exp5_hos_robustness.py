...
#   v1.2  2026-06-01  Added background Drive heartbeat thread (start_drive_heartbeat).
#                     Uploads all checkpoint files to Drive every
#                     DRIVE_HEARTBEAT_INTERVAL seconds throughout the run.
#                     Eliminates data loss on full Colab session wipe.
#   v1.3  2026-06-01  Added Drive-to-local recovery at the start of
#                     extract_features_batch() and train_xgboost():
#                     if local file is missing, download from Drive before
...
#   v1.5  2026-06-01  Removed HDF5 Drive recovery from extract_features_batch().
#                     Downloading partial HDF5 from Drive is unsafe regardless
...
#                     purges from Drive, then refits cleanly. No more crash on
...
#                     uploaded to Drive after every batch. Resumable at
...
DRIVE_HEARTBEAT_INTERVAL = 180   # seconds between periodic checkpoint uploads (3 min)
H5_UPLOAD_EVERY        = 500   # upload HDF5 to Drive after every N utterances (close→upload→reopen)
...
DRIVE_BACKUP_DIR = PROJECT_DIR  # HDF5 backup in project folder
...
# =============================================================================
# SECTION 1 — Google Drive Helper Functions
# =============================================================================
...
def save_to_drive(local_filepath: str, remote_filename: str, retries: int = 3):
    ...
    for attempt in range(retries):
        try:
            ...
            log.info(f"  Drive upload OK: {remote_filename}")
            return
        except Exception as e:
            log.warning(f"  Drive upload attempt {attempt+1} failed: {e}")
            time.sleep(2)
    log.error(f"  Drive upload FAILED after {retries} attempts: {remote_filename}")

def load_from_drive(remote_filename: str, local_filepath: str) -> bool:
    ...
    if os.path.exists(src_path):
        try:
            ...
            log.info(f"  Drive download OK: {remote_filename}")
            return True
        except Exception as e:
            log.warning(f"  Drive download failed: {e}")
            return False
    else:
        log.info(f"  Drive file not found: {remote_filename}")
        return False

def drive_file_exists(remote_filename: str) -> bool:
    """Check if file exists in Google Drive project folder."""
    ensure_project_dir()
    src_path = os.path.join(PROJECT_DIR, remote_filename)
...
def list_drive_files() -> List[str]:
    ...
    try:
        ...
        log.warning(f"Could not list Drive files: {e}")
        return []

# =============================================================================
# SECTION 1b — Background Drive Heartbeat Thread
# =============================================================================
...
def _heartbeat_worker(ckpt_dir: str, interval: int, stop_event: threading.Event):
    ...
    to Google Drive. HDF5 files (.h5) are intentionally EXCLUDED — they are uploaded
    ...
    """
    log.info(f"[Heartbeat] Drive backup thread started  (interval={interval}s)")
    ...
    log.info("[Heartbeat] Drive backup thread stopped.")


def start_drive_heartbeat(ckpt_dir: str,
                        interval: int = DRIVE_HEARTBEAT_INTERVAL
                        ) -> Tuple[threading.Thread, threading.Event]:
    """
    Starts the background Drive heartbeat thread.
    Returns (thread, stop_event).
    Call stop_event.set() to cleanly shut the thread down at the end of main().

    Usage:
        thread, stop_evt = start_drive_heartbeat(CHECKPOINT_DIR)
        ... run experiment ...
        stop_evt.set()
        thread.join()
    """
...
def discover_audio_root(search_root: str) -> str:
    ...
    for dirpath, dirnames, _ in os.walk(search_root):
        ...
        # (names may vary across zip versions)
...
def recover_h5(h5_path: str) -> int:
    ...
    Validates every candidate before accepting it — corrupt
    or empty files are deleted immediately.

    Returns number of utterances recovered (0 = starting fresh).

    Recovery priority:
      1. Drive backup  (PROJECT_DIR/<filename>)
    """
...
def extract_features_batch(utt_ids: List[str], labels_dict: Dict[str, int],
    ...
    errors        = 0
    since_upload  = 0   # utterances written since last Drive upload of this HDF5

    # We open/close h5 manually (not via 'with') so we can close it safely
    # before each Drive upload, then reopen it.
    ...
    try:
        for i, utt_id in enumerate(tqdm(todo, desc=f"Features [{partition}]",
            ...
            if since_upload >= H5_UPLOAD_EVERY:
                ...
                # Drive copy
                try:
                    save_to_drive(h5_path, remote_name)
                    log.info(f"[H5 checkpoint] Drive: {remote_name} "
                             f"({os.path.getsize(h5_path)/1e6:.0f} MB) "
                             f"— {n_so_far} utterances")
                except Exception as e:
                    log.warning(f"[H5 checkpoint] Drive upload failed: {e}")
                hf = h5py.File(h5_path, 'a')
                since_upload = 0
    finally:
        hf.flush()
...
def generate_and_extract_condition(
    ...
    Processes utterances in batches of `batch_size`:
      For each batch:
        ...
        4. Checkpoint HDF5         (Drive upload after each batch)
    ...
    for b_idx in range(n_batches):
        ...
        if batch_feats:
            ...
            try:
                save_to_drive(h5_path, remote_name)
                log.info(f"  [{cond_tag}] Drive checkpoint: {len(done_ids)} utterances "
                         f"({os.path.getsize(h5_path)/1e6:.0f} MB)")
            except Exception as e:
...
def train_xgboost(X_train: np.ndarray, y_train: np.ndarray,
    ...
    or on Drive (downloads on session wipe)."""
    # --- Drive recovery: restore model if local disk was wiped ---
    if not os.path.exists(model_path):
        remote_name = os.path.basename(model_path)
        log.info(f"Local model not found — attempting Drive recovery: {remote_name}")
        if load_from_drive(remote_name, model_path):
            log.info(f"  Recovered model from Drive ({os.path.getsize(model_path)/1e3:.1f} KB)")
        else:
            log.info(f"  No Drive copy found. Will train from scratch.")
...
def run_lfcc_gmm_baseline(eval_utt_ids: List[str], labels_dict: Dict[str, int],
    ...
    with open(scores_path, 'w') as f:
        json.dump(result, f)
    save_to_drive(scores_path, os.path.basename(scores_path))
...
def _fit_lfcc_gmm_if_needed(train_utt_ids: List[str],
    ...
    if _GMM_BONAFIDE is not None:
        return
    # --- Drive recovery: restore GMM if local disk was wiped ---
    if not os.path.exists(gmm_path):
        log.info("Local GMM not found — attempting Drive recovery: lfcc_gmm_models.npz")
        if load_from_drive("lfcc_gmm_models.npz", gmm_path):
            log.info(f"  Recovered GMM from Drive ({os.path.getsize(gmm_path)/1e6:.1f} MB)")
        else:
            log.info("  No Drive copy found. Will refit GMM.")
    if os.path.exists(gmm_path):
        try:
            ...
            os.remove(gmm_path)
            # Also purge from Drive so recovery does not re-download corrupt copy
            drive_src = os.path.join(DRIVE_BACKUP_DIR, "lfcc_gmm_models.npz")
            if os.path.exists(drive_src):
                os.remove(drive_src)
                log.info("Deleted corrupt lfcc_gmm_models.npz from Drive.")
            except Exception:
                pass
    ...
    save_to_drive(gmm_path, "lfcc_gmm_models.npz")
...
def main():
    ...
    # ------------------------------------------------------------------
    # Step 0: Start background Drive heartbeat
    # ------------------------------------------------------------------
    hb_thread, hb_stop = start_drive_heartbeat(CHECKPOINT_DIR,
                                              interval=DRIVE_HEARTBEAT_INTERVAL)
    ...
    save_to_drive(train_h5, "hos_features_train.h5")
    ...
    save_to_drive(eval_clean_h5, "hos_features_eval_clean.h5")
    ...
    if loaded is None and load_from_drive("results_exp5.json", results_path):
        loaded = _safe_load_results(results_path)
    ...
    for (cond_tag, ffmpeg_args, human_label) in DISTORTIONS:
        ...
        if os.path.exists(lfcc_scores_cond_path):
            ...
            save_to_drive(lfcc_scores_cond_path,
        ...
        save_to_drive(results_path, "results_exp5.json")
    ...
    if mp3_hos:
        ...
        plot_eer_vs_bitrate(mp3_tags, mp3_hos, mp3_lfcc, mp3_rates,
                             "MP3", fig1)
        save_to_drive(fig1, "fig_05_01_eer_vs_mp3.png")
    ...
    if opus_hos:
        ...
        plot_eer_vs_bitrate(opus_tags, opus_hos, opus_lfcc, opus_rates,
                             "Opus", fig2)
        save_to_drive(fig2, "fig_05_02_eer_vs_opus.png")
    ...
    if awgn_hos:
        ...
        save_to_drive(fig3, "fig_05_03_eer_vs_awgn.png")
    ...
    if ordered_tags:
        ...
        save_to_drive(fig4, "fig_05_04_degradation_radar.png")
    ...
    plot_eer_heatmap(cond_lbls, ["HOS-XGBoost", "LFCC-GMM"],
                     eer_matrix, fig5)
    save_to_drive(fig5, "fig_05_05_eer_heatmap.png")
    ...
    log.info("=" * 70)
    log.info("All outputs uploaded to Drive. Experiment 5 complete.")

    # ------------------------------------------------------------------
    # Step 11: Stop background heartbeat thread cleanly
    # ------------------------------------------------------------------
    log.info("[Heartbeat] Stopping Drive backup thread …")
    hb_stop.set()
    hb_thread.join(timeout=30)
    log.info("[Heartbeat] Done.")

    # ------------------------------------------------------------------
    # Step 12: Sync to ensure all writes are flushed
    # ------------------------------------------------------------------
